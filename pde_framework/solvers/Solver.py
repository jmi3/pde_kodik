    ...
    def run(
        ...
        *,
        ...
        if time_slices is None:
            time_slices = []
        slices_data = [np.zeros(shape=(n_steps+1,), dtype=np.complex128) for _ in time_slices]
        slices_indices = [initial.grid.find_nearest_index(slice_pos) for slice_pos in time_slices]
        ...
        time_grid = Grid1D(x_min=0, x_max=dt_value*n_steps, n_points=n_steps+1)
        self.time_slices = [ScalarField(grid=time_grid, data=slice_data) for slice_data in slices_data]
